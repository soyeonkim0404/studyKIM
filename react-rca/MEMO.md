...
## 리덕스 미들웨어 (// 이건 좀더 공부해서 말씀드리겠습니다.)
주로 비동기 작업을 처리할때 사용한다. 
리덕스 미들웨어는 리덕스를 사용하는 가장 큰 이유 중 하나라고 한다. 리덕스 미들웨어를 사용하지 않을거면, 그냥 context API(Context API는 리액트에 내장된 기능이고 리덕스와 비슷)를 사용하라고 할 정도 이다.
